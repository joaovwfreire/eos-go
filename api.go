...
func (api *API) GetAccountTxHistory(ctx context.Context, name AccountName, opts ...GetAccountOption) (out *AccountResp, err error) {
	body := M{"account_name": name}
	for _, opt := range opts {
		opt.apply(body)
	}

	err = api.call2(ctx, "history", "get_actions", body, &out)
	return
}
...
func (api *API) call2(ctx context.Context, baseAPI string, endpoint string, body interface{}, out interface{}) error {
	jsonBody, err := enc(body)
	if err != nil {
		return err
	}

	targetURL := fmt.Sprintf("%s/v2/%s/%s", api.BaseURL, baseAPI, endpoint)
	req, err := http.NewRequest("GET", targetURL, jsonBody)
	if err != nil {
		return fmt.Errorf("NewRequest: %w", err)
	}
